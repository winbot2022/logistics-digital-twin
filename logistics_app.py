...
# URLに値があればそれを使う。なければデフォルト値(60, 3, 2.5)を使う。
...
# --- 入力ウィジェットの初期値にセットする ---
avg_orders_per_hour = st.sidebar.number_input("平均注文数", value=default_orders)
num_packers = st.sidebar.slider("スタッフ数", 1, 10, value=default_staff)
avg_packing_time = st.sidebar.number_input("平均梱包時間", value=default_time)
...
avg_orders_per_hour = st.sidebar.number_input("1時間あたりの平均注文数 (件)", value=60, step=5)
arrival_interval = 60.0 / avg_orders_per_hour # 分単位に変換

st.sidebar.subheader("2. 現場のキャパシティ")
num_packers = st.sidebar.slider("出勤する梱包スタッフ数 (人)", 1, 10, 3)
avg_packing_time = st.sidebar.number_input("1件あたりの平均梱包時間 (分)", value=2.5, step=0.1)
