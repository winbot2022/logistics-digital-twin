...
# 免責事項を小さめの文字（caption）で表示
st.sidebar.markdown("---")
st.sidebar.caption("""
**【免責事項】**
本ツールは理論上のシミュレーション値であり、実際の現場環境を保証するものではありません。判断の参考値としてご活用ください。
""")

# ボタンを「実行」に絞り、横幅いっぱいに広げて誤タップを防ぐ
if st.sidebar.button("シミュレーションを実行", use_container_width=True):
    ...
    with col_a:
        st.info("**本格導入相談**\n\n実データに基づく専用モデルを構築します。")
        # ボタンを横幅いっぱいに
        st.link_button("無料相談・予約", "https://your-contact-form-url.com", use_container_width=True)
    
    with col_b:
        st.success("**無料トライアル**\n\n人員配置を最適化する本格ダッシュボードを提供。")
        # ボタンを横幅いっぱいに
        st.link_button("詳細を見る", "https://your-service-page.com", use_container_width=True)
